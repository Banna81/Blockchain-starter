...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (errorLog.length !==0){
                reject("chain is not valid");
                 ...
                 // block height
                const currentHeight = await self.chain.length;
                // UTC timestamp
                block.timeStamp = new Date().getTime().toString().slice(0,-3);
                block.height = currentHeight + 1;
                if (this.chain.length>0) {
                // previous block hash
                block.previousHash = this.getLatestBloezick().hash;
                }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            reject(Error("problem in verifying"));
            }
            } else {
            reject(Error("Rejct ,Taking longer than 5 minutes"))
            }
    ...
    getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
            let ownedBlocks = self.chain.filter(p => p.owner === address);
            if (ownedBlocks.length === 0){
                reject('Wallet Address cannot be found.');
            } 
            stars = ownedBlocks.map(p => JSON.parse(hex2ascii(p.body)));
            if(stars) {
               resolve(stars);
            }  
            else{
               reject('Stars cant be returned'); 
            } 
